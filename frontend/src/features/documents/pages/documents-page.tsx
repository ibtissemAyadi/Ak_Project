...
export function DocumentsPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Documents"
        description="Store, organize and version project and client documents."
        actions={
            ...
            <Card key={doc.id} className="group cursor-pointer transition-shadow hover:shadow-md" onClick={() => setPreviewDoc(doc)}>
              <CardContent className="space-y-3 p-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">{(doc.sizeKb / 1024).toFixed(1)} MB · v{doc.versions.length}</p>
                </div>
                <div className="flex items-center justify-between">
                  <Badge variant="secondary">{DOCUMENT_CATEGORY_META[doc.category]?.label}</Badge>
                  <span className="text-[11px] text-muted-foreground">{formatDate(doc.updatedAt)}</span>
                </div>
              </CardContent>
            </Card>
          ))}
